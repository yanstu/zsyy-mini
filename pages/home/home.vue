	...
	<view class="container" :style="{ height: bgHeight + 'px' }" @click="changes(change)">
		...
		<view class="lunbo" :style="{ height: bgHeight + 'px' }">
			<swiper :circular="true" :autoplay="true" :interval="3000" :duration="1000">
				<swiper-item v-for="(item, index) in bgImage" :key="index"><image class="image" :src="item.url"></image></swiper-item>
			</swiper>
		</view>

		<!-- 搜索 - 天气 -->
		<view class="search" :style="{ top: top + 'px' }">
			<view v-if="weather.length == 0 ? false : true" style="width: 100px; height: 32px; position: absolute; top: -2px; left: -105px; text-align: center;">
			...
			<view class="box-menu-search" @click.stop="openTab">
				...
				<input class="box-menu-search-input" type="text" placeholder="请输入目的地" placeholder-style="color:rgb(232,240,252);" />
			</view>
		</view>
		...
		<view class="service">
			<ul>
				<li v-for="(item, index) in menuList" :key="index" @click.stop="goLowerlevel(index, change)">
					<view class="service-bg" :style="{ background: item.color }"><image :src="item.url" mode="aspectFill"></image></view>
					<text>{{ item.name }}</text>
				</li>
			</ul>
		</view>
			...
			<ul>
				<view style="overflow: auto; height: 68%; position: absolute; width: 100%; top: 8%;">
					<li v-for="(item, index) in more" :key="index" @click.stop="moreMenu(item.name, item)">
						<view>
							<view class="service-bg service-bg-change" style="width: 20%;" :style="{ background: item.bgColor }">
				...
				<view style="position: absolute; width: 100%; bottom: 8%; display: flex;">
					<li class="bianji" style="border-right: 1px dashed rgba(118,118,118,0.3); padding-left: 5%;">
						...
						<view class="service-bg service-bg-change" style="background: rgba(254,114,97,0.5);">
							<image src="https://kshht.gzmsdw.cn/imge_zsyy/html/home_more/客服.png" mode="aspectFill"></image>
						</view>
						...
						<!-- #ifdef  MP-ALIPAY -->
						<contact-button tnt-inst-id="1Nc_16I2" scene="SCE01045292" color="#EDACAB" size="72rpx"></contact-button>
					...
					<li class="bianji" style="padding-left: 6%;" @click.stop="nextPage">
						<view class="service-bg service-bg-change" style="background: rgba(124,185,254,0.5); ">
							<image src="https://kshht.gzmsdw.cn/imge_zsyy/html/home_more/发票.png" mode="aspectFill"></image>
						</view>
						<view class="banji-text" style="padding-left: 27%; bottom: 0;"><text class="text">编辑</text></view>
					</li>
				</view>
			</ul>
		</view>
...
export default {
	...
	onShow(e) {
		...
		if (currPage.data.more == undefined || currPage.data.title == '') {
		} else {
