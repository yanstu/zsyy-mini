<template>
	<view>
		...
		<view class="btns">
			<!-- #ifdef MP-WEIXIN -->
			<button :loading="loginLoading" class="tobtn" type="primary" v-if="!isLogin" @click="getUserInfo" withCredentials="true">微信登录</button>
			...
			<button :loading="phoneLoading" class="tobtn" scope="phoneNumber" open-type="getAuthorize" @error="errorAuthorize" @getAuthorize="getAlihoneNumber">手机号授权</button>
			<!-- #endif -->
...
export default {
	...
	methods: {
				...
				success: res => {
					uni.getUserInfo({
						...
						success: result => {
							...
							// 支付宝登录
							// #ifdef MP-ALIPAY
							this.login(JSON.parse(result.response), res.code);
							// #endif
		...
		login(code, codes) {
			...
			// #ifdef MP-ALIPAY
			this.$req.request({
				url: '/api/zfb/zfbLogin',
				method: 'GET',
				data: {
					avatar: code.response.avatar,
					city: code.response.city,
					code: codes,
					countryCode: code.response.countryCode,
					gender: code.response.gender,
					nickName: code.response.nickName,
					province: code.response.province
				},
				success: res => {
					if (res.data.code == 200) {
						...
						uni.setStorageSync('level', res.data.data.level); //商家状态码
