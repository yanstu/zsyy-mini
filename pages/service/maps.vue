<template>
	<view class="maps-container">
			...
			<view class="search-bar-form">
				<view class="search-bar-box">
					...
					<input
						...
						@input="inputTyping"
					/>
					<view class="icon-clear" v-if="inputVal" @tap="clearInput"><icon type="clear" size="14"></icon></view>
				</view>
		...
		<scroll-view scroll-y class="scrollView" :style="{ height: scrollH + 50 + 'px' }" @scrolltolower="pullUp">
			<view class="tui-list">
					...
					<view class="addr-box ">
						...
						<view class="addr-opera ">
							<view class="opera-box " hover-class="tui-opcity" :hover-stay-time="150" @tap="call" :data-id="item.id" v-if="item.tel">
								<image src="https://kshht.gzmsdw.cn/imge_zsyy/html/my/call.png" class="mini-img"></image>
								<view class="text">打电话</view>
							</view>
							<view class="opera-box " hover-class="tui-opcity" :hover-stay-time="150" @tap="go" :data-id="item.id">
								<image src="https://kshht.gzmsdw.cn/imge_zsyy/html/my/go.png" class="mini-img"></image>
...
<script>
const QQMapWX = require('@/js_sdk/qqmap/qqmap-wx-jssdk.js');
...
export default {
	...
	onLoad(options) {
		...
		this.lng = uni.getStorageSync('lot');
		this.qqmapsdk = new QQMapWX({
			key: this.key
		});
		this.keywords = options.key || '加油站';
		uni.setNavigationBarTitle({
			title: options.key || '加油站'
		});
		setTimeout(() => {
	...
	methods: {
		...
		getResult(data) {
			...
			for (let [index, item] of data.entries()) {
				arr.push({
					...
					title: item.title,
					iconPath: 'https://kshht.gzmsdw.cn/imge_zsyy/html/maps/location.png',
					width: 32,
					height: 32
				});
		...
		getPoiAround(keywords) {
			...
			let url = `https://apis.map.qq.com/ws/place/v1/search?keyword=${keywords}&boundary=${boundary}&page_size=20&page_index=${this.pageIndex}&output=jsonp&key=${this.key}`;
			this.tui.tuiJsonp(
				url,
				res => {
					if (res.status === 0) {
